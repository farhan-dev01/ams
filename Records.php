...
$querytable= mysqli_query($conn,"SELECT a.date,e.eid,a.status from attendance as a join employee as e on a.eid = e.eid JOIN department as d on e.deptno = d.deptno where a.date BETWEEN '".($year."-".($nmonth))."-26' and '$date' order by a.eid,a.DATE ");

$abc="SELECT a.date,e.eid,a.status from attendance as a join employee as e on a.eid = e.eid JOIN department as d on e.deptno = d.deptno where a.date BETWEEN '".($year."-".($nmonth))."-26' and '$date' order by a.eid,a.DATE ";
...
<header>
    ...
    <div class="pageWrapper bg-light h-50">
        <section class="pageContent container">
                        ...
                        <form class="filterForm col-md-8" method="POST" action="#">
                            <div class="row">
                                <div class="form-group col-md-4">
                                    ...
                                    <input class="form-control round monthEnd dateIcon date-picker2" readonly type="text"  id="monthEnd" name="date" placeholder="" value="<?php
                                    $d = explode(' ', date('F Y'));
                                    $month = $d[0];
                                    $year = $d[1];
                                    ...
                                    <select name="dname" id="department">
                                        ...
                                        $conn = opencon();
                                        {
                                            ...
                                            while ($result = mysqli_fetch_array($query)) {
                                                ...
                                                ?>
                                                <option value="<?php echo $departname; ?>"
            ...
            <form method="post" id="#forexcel" action="ExportToExcel/ExportFiltered.php" target="_blank">
                <section class="">
                            ...
                            <div id="page">
                                ...
                                $conn = opencon();
                                {
                                    ...
                                    if (isset($_POST['submit'])) {
                                        $ename = $_POST['name'];
                                        $dname = $_POST['dname'];
                                        $q="SELECT employee.*,department.dname from employee join department on employee.deptno=department.deptno where employee.ename like '%" .$ename . "%' and department.dname like '%" . $dname . "%' order by employee.eid limit $start,$per_page ";
                                        $query = mysqli_query($conn,
                                            $q);
                                    } else {
                                        $q="SELECT employee.*,department.dname FROM employee join department on employee.deptno=department.deptno order by employee.eid limit $start,$per_page";
                                        $query = mysqli_query($conn,
                                            $q) or
                                        ...
                                        <ul class="employeeList clearfix">
                                            <li class="bg-white round boxShadow">
                                                <ul class="employeeListDesc clearfix align-middle">
                                                    ...
                                                    $year =$d[1];
                                                    $nmonth = date("m", strtotime($year.'-'.$month.'-1'))-1;
                                                    if(!isset($_POST['date'])  && date('d')>=26)
                                                    {
                                                        $nmonth+=1;

                                                    }
                                                    //   echo'<script>alert('.$nmonth.')</script>';
                                    ...
                                    echo ' <input type="hidden" value="'.$ename.'" name="ename" /><input type="hidden" value="'.$dname.'" name="dname" /><input 
type="hidden" value="'.date_format(date_sub(date_create($dateglobal), date_interval_create_from_date_string('1 months')),'Y-m').'-26'.'" name="dateRange" /> ';
            ...
            {
            ?>
            <a  href="javascript:void(0)" onclick="document.getElementById('#forexcel').submit();" class="btn btn-primary px-5 border-0 roundBtn float-right">Export
                <?php
                } else
                {
                ?>
                <a   href="" data-toggle="modal" data-target="#myModal" class="btn btn-primary px-5 border-0 roundBtn float-right">Export
                    <?php
                    }
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                Are you Sure you want to export all the records?
                            </div>
                            <div class="modal-footer">
                                <form action = "#">
                                    <input type="hidden" value="" id = "sid" name = "id"/>
                                    <a  id= "exp" href="ExportToExcel/Export.php?year=<?php echo date('Y'); ?>" class="btn btn-success roundBtn" data-window="external" data-dismiss="modal" >Export</a>
                                    <input type="button" data-dismiss="modal" class="btn btn-danger roundBtn"  value = "No"/>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <script>
                    $("#myModal").on('click',"a[data-window='external']", function() {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            $("#emp").append("<option value='" + name + "'>" + name + "</option>");
