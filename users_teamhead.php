...
$conn=opencon();
$per_page = 10;
$pages_query = mysqli_query($conn,"select count('uid') from teamhead_user");
...
<?php 
  ...
  ?>
        ...
        <section class="pageTitle row">
            ...
            <div class="col-md-2">
                <a href="javascript:void(0)" onclick="window.location.href='add-user_teamhead.php'" class="btn btn-primary roundBtn float-right">+ ADD USER</a>
            </div>
        </section><!-- Page Title-->
            ...
            <div class="allEmployeeList">
                ...
                <ul class="row list clearfix d-block m-0 p-0 ">
                    <li class="tex-center row align-items-center justify-content-center">
                        ...
                        <div class="col-sm-1 col-md-1 align-self-center text-left">DEPARTMENT</div>
                        <div class="col-sm-1 col-md-2 align-self-center text-left">USERNAME</div>
                        <div class="col-sm-1 col-md-1 align-self-center text-left">PASSWORD</div>
 ...
 $conn=opencon();
 {
	 ...
	 $query = mysqli_query($conn, "SELECT teamhead_user.*,department.dname FROM teamhead_user left outer join department on teamhead_user.deptno=department.deptno limit $start,$per_page ") or die(mysqli_error($conn));
     $sno=1;
 while ($result=mysqli_fetch_array($query))
    {
        ...
        <form>
                    <li class="tex-center row align-items-center justify-content-center bg-white round boxShadow">
                        <div class="col-sm-1 col-md-1 align-self-center text-left"><img id ="pic" src=<?php echo $result['uimage'] ?> class="rounded-circle" alt="" /></div>
                        ...
                        <div class="col-sm-1 col-md-1 align-self-center text-left holidayName thumbnail"><?php echo $result['dname'] ?></div>
                        <div class="col-sm-1 col-md-2 align-self-center text-left holidayName thumbnail"><?php echo $result['username'] ?></div>
                        <div class="col-sm-1 col-md-1 align-self-center text-left holidayName thumbnail"><?php echo $result['password'] ?></div>
                        <a class="col-sm-1 col-md-1  align-self-center text-center" href="edit-user_teamhead.php?id=<?php echo $result['uid'] ?>;"><img src="content/img/edit.png" class="" alt="" /></a>
                        <a class="col-sm-1 col-md-1  align-self-center text-center" href="" onclick = "document.getElementById('sid').value =  <?php echo $result['uid'] ?>;"
...
if(isset($_GET['id']))
{
    ...
    $conn=opencon();
    $ID = $_GET['id'];
    $query = $conn->query("SELECT username FROM teamhead_user WHERE uid='$ID'") or die(mysqli_error());
    $row=$query->fetch_array();
    $uname=$row['username'];
    $sql = "DELETE FROM teamhead_user WHERE uid='$ID'";
